...
- 후보 하나는 `{total_time_min, fare, transfer_count, total_walk_m, modes, legs}` 다.
  `modes` 는 그 경로에 실제로 등장한 이동수단만 지하철·시내버스·고속버스·시외버스
  순으로 담아, 목록 화면이 후보를 열지 않고도 아이콘을 고를 수 있게 한다.
- 구간 종류(`legs[].type`)는 `walk`·`subway`·`bus`·`express`·`intercity` 다섯이다.
  `express` 는 고속버스(`trafficType=5`), `intercity` 는 시외버스(`=6`)다. 이 값을
  따로 두지 않으면 도시 간 이동이 통째로 도보로 떨어진다 — 안산 → 속초 234 km 가
  "도보 217분"으로, 서울 → 부산은 후보 대부분이 "도보 190 km"로 나왔다. 시내버스로
  합치지 않는 것은 요금대와 정류장 표기가 서로 달라서고, 고속버스와 시외버스를 다시
  가르는 것은 터미널과 요금 체계가 달라서다. 다만 **거리 비중을 낼 때는 셋 다 버스로
  합산한다** — 비중은 "지하철 경로냐, 사실상 버스 경로냐"를 가르는 값이라 시내든
  도시 간이든 지하철이 아니라는 점에서 같고, 빼면 도시 간 경로가 분모 0 이 되어
  버스 비중 0%(곧 "지하철 위주")로 잘못 읽힌다.
- `train`(열차, `trafficType=4`)·`air`(항공, `=7`)도 도보로 뭉개지 않는다. 다만 거리
  비중의 어느 쪽에도 넣지 않고, **지하철·버스 버튼 양쪽에서 뺀다**. 지하철 거리 0 만
  보고 "버스"로 치면 KTX 가 버스 목록 맨 위에 오고, 비중만 보면 열차 단독 후보가
  "버스 비중 0%"라 지하철 목록에 샌다. 서울 → 부산은 후보 19건 중 열차 10·항공 1이라
  그냥 두면 두 버튼이 전부 오염된다.
- **상한(`ROUTE_OPTIONS_MAX`)은 거른 뒤에 적용한다.** 먼저 자르면 버튼과 맞는 후보가
  상한 밖으로 밀려난다 — 서울 → 부산은 빠른 순으로 열차·항공이 앞 8칸을 채워, 고속버스
  4건·시외버스 4건이 있는데도 버스 버튼이 "갈 수 있는 경로가 없어요"였다.
